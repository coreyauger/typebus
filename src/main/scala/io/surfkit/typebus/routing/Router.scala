...
trait Router {
  ...
  def addRouteRule[T : ClassTag](dst: ActorRef*)(filter: (T) => Boolean)(trans: (T) => Any = (t:T) => t ) = {
    def ruleFun = (msg: T) => {
      if(filter(msg)) {
  ...
  /*def route[T : ClassTag](msg: T) = listOfRoutes.map{ f =>
    if(f.asInstanceOf[PartialFunction[AnyVal,String]].isDefinedAt(msg))
      f.asInstanceOf[PartialFunction[T, String]](msg)
  }*/


  protected[this] def funToPF[T : ClassTag](f: (T) => Boolean) = new PartialFunction[T, Boolean] {
